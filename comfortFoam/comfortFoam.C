...
(
    const fvMesh& mesh_,
    const List<scalar>& cellSetList_
)
{
    ...
    forAll (cellSetList_, labelI)
    {
        const int cellI = cellSetList_[labelI];
...
int main(int argc, char *argv[])
{
    ...
    //- Include cell ID's of cell set
    List<scalar> cellSetList;

    if (args.optionReadIfPresent("cellSet", cellSetName))
    {
        Info<< "Reading cell set from " << cellSetName << endl;    

        cellSet tmp(mesh, cellSetName); 

        //- Add all cells that are in the cellSet to cellList
        forAll(mesh.C(), cellI)
        {
            if (tmp[cellI])
            {
                cellSetList.append(cellI);
            }
        }
    }
    //- If no cell set is used, all cells are used for calculation
    else
    {
        forAll(mesh.C(), cellI)
        {
...
//            volScalarField Qr(THeader, mesh);
        ...
        vector wl = Uaverage(mesh, cellSetList);
        scalar Tr = Taverage(mesh, cellSetList);
        ...
        forAll (cellSetList, labelI)
        {
            const int cellI = cellSetList[labelI];
